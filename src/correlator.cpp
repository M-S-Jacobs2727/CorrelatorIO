...
std::vector<double> Correlator::computeTimesteps(double timestep)
{
    ...
    timesteps.resize(result.size());

    uint32_t k = 0;
    for (; k < m_points_per_corr; ++k)
        timesteps[k] = k * timestep;
...
void Correlator::evaluate()
{
    uint32_t k = 0;
    for (; k < m_points_per_corr; ++k)
        if (m_num_vals_mat[0][k] != 0)
            result[k] = m_correlation_mat[0][k] / m_num_vals_mat[0][k];

    for (uint32_t i = 1; i < m_num_correlators_used; ++i)
        for (uint32_t j = m_min_dist_bt_points; j < m_points_per_corr; ++j)
